...
def _login_max_attempts() -> int:
    return int(os.getenv("ZMART_EDGE_LOGIN_MAX_ATTEMPTS", "5"))


def _login_lockout_seconds() -> int:
    return int(os.getenv("ZMART_EDGE_LOGIN_LOCKOUT_SECONDS", "900"))
...
def _token_ttl_hours() -> int:
    return int(os.getenv("ZMART_EDGE_ACCESS_TOKEN_TTL_HOURS", "24"))
...
def _invite_default_ttl_hours() -> int:
    return int(os.getenv("ZMART_EDGE_INVITE_DEFAULT_TTL_HOURS", "168"))
...
def authenticate_emergency_token(token: str) -> AuthenticatedUser | None:
    enabled = os.getenv("ZMART_EDGE_ENABLE_EMERGENCY_TOKEN", "0").strip().lower() in {"1", "true", "yes", "on"}
    if not enabled:
        return None
