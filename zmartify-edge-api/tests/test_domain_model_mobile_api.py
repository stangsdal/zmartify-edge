...
def test_ingest_dedup_and_rate_limit(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("ZMART_EDGE_INGEST_MIN_INTERVAL_MS", "999999")
    client = _client(monkeypatch, tmp_path)
    headers = {"Authorization": "Bearer emergency-token"}
...
def test_mobile_api_hides_internal_database_ids(monkeypatch, tmp_path: Path):
    ...
    assert mobile_site.status_code == 200
    assert mobile_site.json()["devices"][0]["device_type"] == "hvac_ahc9000"
    ...
    assert site_devices.json()["devices"][0]["device_type"] == "hvac_ahc9000"
