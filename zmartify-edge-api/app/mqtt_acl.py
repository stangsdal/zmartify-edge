...
def _acl_output_path() -> Path:
    return Path(os.getenv("ZMART_EDGE_MQTT_ACL_FILE", "/mosquitto/config/acl"))
...
def generate_acl_file(conn: sqlite3.Connection) -> str:
    ...
    dry_run = _bool_env("ZMART_EDGE_DRY_RUN_ACL_WRITE", False) or (not should_apply_external_commands())
