...
def normalize_mosquitto_file_permissions(path: str | Path) -> None:
    ...
    uid = _int_env("HVAC_EDGE_MQTT_FILE_UID", 1883)
    gid = _int_env("HVAC_EDGE_MQTT_FILE_GID", 1883)
    mode = _int_env("HVAC_EDGE_MQTT_FILE_MODE", 0o600)

    # Always enforce file mode; ownership may be best-effort in unprivileged envs.
    os.chmod(target, mode)

    strict_owner = os.getenv("HVAC_EDGE_STRICT_FILE_OWNERSHIP", "0").strip() in {"1", "true", "yes", "on"}
    try:
    ...
    docker_cmd = os.getenv("HVAC_EDGE_DOCKER_BIN", "docker")
    helper_image = os.getenv("HVAC_EDGE_CHOWN_HELPER_IMAGE", "alpine:latest")
