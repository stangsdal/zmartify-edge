...
def should_apply_external_commands() -> bool:
    return _bool_env("ZMART_EDGE_APPLY_MQTT_COMMANDS", False)
...
def create_or_update_mqtt_user(username: str, password: str) -> None:
    ...
    passwd_file = os.getenv("ZMART_EDGE_MQTT_PASSWD_FILE", "/mosquitto/config/passwd")
    mosquitto_passwd_bin = os.getenv("ZMART_EDGE_MOSQUITTO_PASSWD_BIN", "mosquitto_passwd")
...
def reload_broker() -> None:
    ...
    reload_cmd = os.getenv("ZMART_EDGE_MQTT_RELOAD_CMD", "docker kill -s HUP hvac-mosquitto")
    ...
    if result.returncode != 0:
        ...
        restart_cmd = os.getenv("ZMART_EDGE_MQTT_RESTART_CMD")
        if restart_cmd:
